...
class Tokenizer(object):
    def __init__(self, num_words=None, na_value=None, min_freq=1, splitter=None,
        ...
        self._min_freq = min_freq  # 词最少出现的次数
        self._lower = lower
    ...
    def fit_on_texts(self, texts, use_padding=False):
        ...
        if self._splitter is not None:  # for sequence
            ...
            word_counts = Counter(list(texts))  # 统计某个词出现的次数
        self.build_vocab(word_counts)  # word_counts+1

    # 降序排序
    def build_vocab(self, word_counts):
        # sort to guarantee the determinism of index order
        word_counts = sorted(word_counts.items(), key=lambda x: (-x[1], x[0]))  # 根据出现的次数排序，降序
        words = []
        for token, count in word_counts:
            if count >= self._min_freq:
                if self._na_value is None or token != self._na_value:  # 如果出现的次数大于等于最小值并且不等于默认的_na_value，将key加入到words中
                    words.append(token.lower() if self._lower else token)
        if self._num_words:
            words = words[0:self._num_words]
        self.vocab = dict(
            (token, idx) for idx, token in enumerate(words, 1 + self.oov_token))  # 整理成字典序号形式{'6261':1, '4505': 2}
        self.vocab["__OOV__"] = self.oov_token  # todo，多加一个0，self.vocab和 self.vocab_size
        if self.use_padding:
    ...
    def encode_category(self, categories):
        category_indices = [self.vocab.get(x, self.oov_token) for x in
                            categories]  # self.vocab共有的类别，categories所有的数据，category_indices：在类别中的索引号
