...
class FeatureEncoder(object):
    def __init__(self,
        ...
        self.data_dir = os.path.join(data_root, dataset_id)  # 将csv转换后的文件存入的目录
        ...
        self.feature_cols = self._complete_feature_cols(feature_cols)  # 14个特征值进行组装
        self.label_col = label_col
        self.version = version
        self.feature_map = FeatureMap(dataset_id, self.data_dir, version)
        self.encoders = dict()

    # 将特征值拆分为形式，[{'name': 'userid', 'active': True, 'dtype': 'str', 'type': 'categorical'}]
    ...
    def read_csv(self, data_path):
        ...
        dtype_dict = dict((x["name"], eval(x["dtype"]) if isinstance(x["dtype"], str) else x["dtype"])
                          for x in all_cols)
        ddf = self.chuck_csv(data_path, dtype_dict)
        # ddf = pd.read_csv(data_path, dtype=dtype_dict, memory_map=True)  # DataFrame类型
        return ddf

    # 分块读取csv
    def chuck_csv(self, data_path, dtype_dict):
        data = pd.read_csv(data_path, dtype=dtype_dict, memory_map=True, sep=',', engine='python', iterator=True)
        loop = True
        chunkSize = 10000
        chunks = []
        while loop:
            try:
                ...
                chunks.append(chunk)  # total num = len(chunks) * chunkSize
                print("append......", len(chunks))
                if len(chunks) == 1:  # todo  修改数量
                    loop = False
    ...
    # 对数据进行处理，数据为null的话进行填充。拿到的特征值获取active=true的所有列
    def preprocess(self, ddf, fill_na=True):
        ...
        for col in all_cols:
            ...
            if "preprocess" in col and col["preprocess"] != "":  # 对特征值key为preprocess的进行处理
                preprocess_fn = getattr(self, col["preprocess"])
                ddf[name] = preprocess_fn(ddf, name)
        active_cols = [self.label_col["name"]] + [col["name"] for col in self.feature_cols if
                                                  col["active"]]  # 获取active=true的所有列
        ddf = ddf.loc[:, active_cols]  # DataFrame类型，可以获取它的values()，columns
        return ddf

    # 对数据进行填充，如果有na_value填充值就用该值填充，没有的话就是用空字符串
    def _fill_na(self, col, series):
        na_value = col.get("na_value")
        if na_value is not None:
            return series.fillna(na_value)
        elif col["dtype"] in ["str", str]:
            return series.fillna("")  # nan使用空字符串填充
        else:
    ...
    def fit_transform(self, ddf, min_categr_count=1, num_buckets=10, **kwargs):
        ...
        data_array = self.transform(ddf)  # 在fit的基础上进行数据处理（编码）
        return data_array
    ...
    def fit(self, ddf, min_categr_count=1, num_buckets=10, **kwargs):
        ...
        self.save_pickle(self.pickle_file)  # 生成pkl文件
        self.feature_map.save(self.json_file)  # 存储数据到json文件
        logging.info("Set feature encoder done.")

    # 某一个特征，和该特征的数据，对其进行tokenizer
    def fit_feature_col(self, feature_column, feature_values, min_categr_count=1, num_buckets=10):
        ...
        self.feature_map.feature_specs[name] = {"source": feature_source,
                                                "type": feature_type}
        if "min_categr_count" in feature_column:  # 【1】判断特征中是否存在某个属性
        ...
        if feature_type == "numeric":  # 【2】判断特征的类型
            ...
            if encoder == "":
                ...
                if "share_embedding" in feature_column:
                    ...
                    if self.is_share_embedding_with_sequence(name):  # todo
                ...
                self.encoders[name + "_tokenizer"] = tokenizer  # 得到dict存放的是特征_tokenizer,属性为tokenizer中的属性
    ...
    def transform(self, ddf):
        ...
        data_arrays = []  # 存储的是，训练集中所有的数据，在类别中的索引号。（例如100行的某列数据，可能只有25个类别，那么将类别进行排序索引，再计算这100行数据属于哪个索引）
        ...
        data_arrays.append(ddf.loc[:, label_name].values)  # # 将目标值加入到data_arrays中
        data_arrays = [item.reshape(-1, 1) if item.ndim == 1 else item for item in data_arrays]  # reshape维度
        data_array = np.hstack(data_arrays)  # 转为列，第一列userid，最后一列目标值
        return data_array

    def is_share_embedding_with_sequence(self, feature):
        for col in self.feature_cols:
            if col.get("share_embedding", None) == feature and col["type"] == "sequence":  # todo
                return True
        return False
