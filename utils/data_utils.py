...
# 划分数据集
def split_train_test(train_ddf=None, valid_size=0,
...
# 对数据进行处理，取数据，编码等
def build_dataset(feature_encoder, all_data=None, train_data=None, valid_data=None, test_data=None, valid_size=0.1,
                  test_size=0.1, split_type="sequential", **kwargs):
    """ Build feature_map and transform h5 data """
    # Load csv data
    all_ddf = feature_encoder.read_csv(all_data) if all_data else None
    ...
    print("already all_ddf csv")

    # Split data for train/validation/test，划分数据集
    if valid_size > 0 or test_size > 0:
        train_ddf, valid_ddf, test_ddf = split_train_test(all_ddf,
                                                          valid_size, test_size, split_type)

    print("split dataset")

    # 对数据进行处理（空值填充，取出列），feature_encoder字段值的信息，ddf数值信息
    train_ddf = feature_encoder.preprocess(train_ddf)
    train_array = feature_encoder.fit_transform(train_ddf, **kwargs)  # ndarray类型[100,15]
    ...
    print("already train data")

    # Transfrom valid_ddf
    if valid_ddf is not None:
        valid_ddf = feature_encoder.preprocess(valid_ddf)
        valid_array = feature_encoder.transform(valid_ddf)  # 不进行fit
        if block_size > 0:
            block_id = 0
...
def h5_generator(feature_map, stage="both", train_data=None, valid_data=None, test_data=None,
    ...
    if stage in ["both", "train"]:
        ...
        assert len(train_blocks) > 0 and len(valid_blocks) > 0, "invalid data files or paths."  # todo
