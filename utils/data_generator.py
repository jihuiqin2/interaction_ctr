...
class DataGenerator(data.DataLoader):
    def __init__(self, data_path, batch_size=32, shuffle=False, num_workers=1, **kwargs):  # num_workers=0使用主线程
        ...
        self.num_blocks = 1
        self.num_batches = int(np.ceil(len(self.dataset) * 1.0 / self.batch_size))  # 训练一次需要循环的次数
        self.num_samples = len(data_array)  # 特征值的数量
        self.num_positives = data_array[:, -1].sum()  # 目标值为1的数量
        self.num_negatives = self.num_samples - self.num_positives  # 目标值为0的数量
