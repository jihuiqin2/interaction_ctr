...
# =========================================================================
# embedding讲解：https://www.jianshu.com/p/63e7acc5e890
import torch
...
class EmbeddingLayer(nn.Module):
    ...
    def forward(self, X):
        ...
        feature_emb = self.embedding_layer.dict2tensor(feature_emb_dict)  # [64,14,1]
        return feature_emb
...
class EmbeddingDictLayer(nn.Module):
        ...
        for feature, feature_spec in self._feature_map.feature_specs.items():
            if self.is_required(feature):
                ...
                if feature_spec["type"] == "numeric":
                    ...
                    embedding_matrix = nn.Embedding(feature_spec["vocab_size"],  # embedding
                                                    feat_emb_dim,
    ...
    # 特征是否需要，required_feature_columns，not_required_feature_columns
    ...
    def dict2tensor(self, embedding_dict, feature_source=None, feature_type=None):
        if feature_source is not None:
            ...
            return torch.stack(list(embedding_dict.values()), dim=1)  # [64,14,1]，类型转换，torch.stack对维度进行扩张，2维变为3维
