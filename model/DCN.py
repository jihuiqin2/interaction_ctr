...
from layer.interaction import CrossNet
...
class DCN(BaseModel):
    ...
    def forward(self, inputs):
        ...
        cross_out = self.crossnet(flat_feature_emb)  # [b,f*emb]
        if self.dnn is not None:
            dnn_out = self.dnn(flat_feature_emb)  # [b,output_dim]
            final_out = torch.cat([cross_out, dnn_out], dim=-1)  # [b, f*emb+output_dim]
        else:
            final_out = cross_out
