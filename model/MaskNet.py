...
class MaskNet(BaseModel):
    ...
    def forward(self, inputs):
        ...
        if self.emb_norm is not None:
            feat_list = feature_emb.chunk(self.num_fields, dim=1)  # f个[b,1,emb]
            V_hidden = torch.cat([self.emb_norm[i](feat) for i, feat in enumerate(feat_list)],
                                 dim=1)  # 【1】归一化，[b,f,emb]
        else:
...
class ParallelMaskNet(nn.Module):
    ...
    def forward(self, V_emb, V_hidden):  # [b,f*emb]
        ...
        v_out = self.dnn(concat_out)  # 【3】feed-forward layer  [b,1]
        return v_out


# 归一化, Instance-Guided Mask,feed-forward layer
class MaskBlock(nn.Module):
    ...
    def forward(self, V_emb, V_hidden):
        V_mask = self.mask_layer(V_emb)  # 【2】[b,f*emb] Instance-Guided Mask，[b,block_dim]
